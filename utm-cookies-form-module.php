...
function utm_cookies_form_module_page() {
    ...
    function save_utm_params_to_cookie() {
        $utm_source_cookie = isset($_GET["utm_source"]) ?  htmlspecialchars( $_GET["utm_source"]) : '';
        $utm_medium_cookie = isset($_GET["utm_medium"]) ?  htmlspecialchars( $_GET["utm_medium"]) : '';
        $utm_term_cookie = isset($_GET['utm_term']) ?  htmlspecialchars( $_GET["utm_term"]) : '';
        $utm_content_cookie = isset($_GET['utm_content']) ?  htmlspecialchars( $_GET["utm_content"]) : '';
        $utm_campaign_cookie = isset($_GET['utm_campaign']) ?  htmlspecialchars( $_GET["utm_campaign"]) : '';
    
        $utm_source_time   = get_option('utm_source_time', '');
        $utm_medium_time   = get_option('utm_medium_time', '');
        $utm_term_time     = get_option('utm_term_time', '');
        $utm_content_time  = get_option('utm_content_time', '');
        $utm_campaign_time = get_option('utm_campaign_time', '');

        // Якщо значення в куках відсутні, використовуємо значення за замовчуванням
        if ($utm_source_cookie) {
            if (empty($utm_source_time)) {
                $expiration = $utm_source_time; // Отримуємо значення з модуля, або пустий рядок, якщо значення відсутнє
            } else $expiration = time() + 3600; 
            setcookie('utm_source', $utm_source_cookie, $expiration, '/');
        }
        if ($utm_medium_cookie) {
            if (empty($utm_medium_time)) {
                $expiration = $utm_medium_time;
            } else $expiration = time() + 3600; 
            setcookie('utm_medium', $utm_medium_cookie, $expiration, '/');
        }
        if ($utm_term_cookie) {
            if (empty($utm_term_time)) {
                $expiration = $utm_term_time;
            } else $expiration = time() + 3600; 
            setcookie('utm_term', $utm_term_cookie, $expiration, '/');
        }
        if ($utm_content_cookie) {
            if (empty($utm_content_time)) {
                $expiration = $utm_content_time;
            } else $expiration = time() + 3600; 
            setcookie('utm_content', $utm_content_cookie, $expiration, '/');
        }   
        if ($utm_campaign_cookie) {
            if (empty($utm_campaign_time)) {
                $expiration = $utm_campaign_time;
            } else $expiration = time() + 3600; 
            setcookie('utm_campaign', $utm_campaign_cookie, $expiration, '/');
        }
    }
    
    add_action('init', 'save_utm_params_to_cookie');    

    function get_utm_values() {
        ...
        if (!empty($utm_medium)) {
            $utm_values .= "utm_medium = " . $utm_medium . "\n";
        }
        if (!empty($utm_term)) {
            $utm_values .= "utm_term = " . $utm_term . "\n";
        }
        if (!empty($utm_content)) {
            $utm_values .= "utm_content = " . $utm_content . "\n";
        }
        if (!empty($utm_campaign)) {
            $utm_values .= "utm_campaign = " . $utm_campaign . "\n";
        }
        return $utm_values;
    }
